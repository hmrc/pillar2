...
class FinancialServiceSpec extends BaseSpec with Generators with ScalaCheckPropertyChecks {
  ...
  val endDate:   LocalDate = LocalDate.now().plusDays(364) // Explicit type ascription added
  ...
  "splitIntoYearIntervals" - {
    ...
    "return a list of 1 year intervals between two dates" in {
      val startDate: LocalDate = LocalDate.of(2020, 5, 15) // Explicit type ascription added
      val endDate:   LocalDate = LocalDate.of(2023, 9, 10) // Explicit type ascription added
      val result = service.splitIntoYearIntervals(startDate, endDate)

      val expectedResult: List[Years] = List( // Explicit type ascription added
    ...
    "return only 1 interval if date is less than a year" in {
      val startDate: LocalDate = LocalDate.of(2020, 5, 15) // Explicit type ascription added
      val endDate:   LocalDate = LocalDate.of(2020, 9, 10) // Explicit type ascription added
    ...
    "only return the last 7 years if dates are beyond 7 years" in {
      val startDate: LocalDate = LocalDate.of(2020, 1, 1) // Explicit type ascription added
      val endDate:   LocalDate = LocalDate.of(2028, 1, 1) // Explicit type ascription added

      val newAdjustedStartDate: LocalDate = LocalDate.of(2021, 1, 1) // Explicit type ascription added

      val result = service.splitIntoYearIntervals(startDate, endDate)

      result.headOption.map(_.startDate) mustNot be(Some(startDate))
      result.headOption.map(_.startDate) mustBe Some(newAdjustedStartDate)
      result.lastOption.map(_.endDate) mustBe Some(endDate)
    }
  }
}
