...
import uk.gov.hmrc.pillar2.models.hip.{ErrorSummary, SuccessResponse, ValidationErrors}
    ...
    response.status match {
      ...
      case 422 =>
        response.json.validate[ValidationErrors] match {
          case JsSuccess(validationErrors, _) => UnprocessableEntity(Json.toJson(ErrorSummary(validationErrors.code, validationErrors.text)))
          case JsError(_)                     => result_500
        }
      case _ =>
